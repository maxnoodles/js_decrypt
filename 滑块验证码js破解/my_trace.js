...
function get_traces(distance){
    var value = my_random(0.55, 0.75).toFixed(2)
    // 增加回滑
    distance += 20
    var v = 0, t =0.2 , sum = 0;
    var plus = []
    var mid = distance * value
    while (sum < distance){
        if (sum < mid){
            // 指定范围随机产生一个加速度
            a = my_random(2.5, 3.5).toFixed(1)
        }else{
            //指定范围随机产生一个减速的加速度
            a = -my_random(2.0, 3.0).toFixed(1)
        }
        var s = v * t + 0.5 * a * (t ** 2)
        v = v + a * t
        sum += s
        plus.push(Math.round(sum))
    }
    // 回滑数组
    var reduce = [-3, -3, -2, -2, -2, -2, -2, -1, -1, -1]
    for (i of reduce){
    ...
    var trace_array = []
    for(var i=0; i < plus.length; i++){
        // 将 x 偏移, y 偏移, 时间组成一个字符串放回
        trace_array.push(`${plus[i]},${Math.round(my_random(-2, 3))},${time_array[i]}`)
    }
    return trace_array
}


function sample(array, times){
    var arr_len = array.length;
    if (arr_len <= times)
        return array;
    for (var i = [], r = 0, o = 0; o < arr_len; o++){
        o >= r * (arr_len - 1) / (times - 1) && (i.push(array[o]), r += 1);
    }
    return i
}
